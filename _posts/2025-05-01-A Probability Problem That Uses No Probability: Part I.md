...
There are three letters going on so let's keep track of what each one is doing: $x$ and $y$ can be any numbers between 0 and 1 and $(x,y)$ is the coordinate of a point in the square, while $k$ is any integer. Let's try a few values of $k$ to get a feeling for the regions described. $k = 0$ corresponds to the region

$0 \leq y < x$

Which describes a triangle in the lower right corner of our square:
![Triangle](/assets/images/3.1.png)
...
*Area* = *base* $\times$ *height* / 2 = $(1/2 - 1/3)/2$
